...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def process_request(self, request, client_address):
        ...
        to be able to get the thread object which is instantiated and set its start time as an attribute
        """
    ...
    def _handle_request_noblock(self):
        ...
        We override this function to add an additionalcheck before the accept().
        """
...
class ThreadedServer(CommonServer):
    ...
    def process_limit(self):
        ...
        if vms > config['limit_memory_soft']:
            _logger.info('Thread virtual memory limit (%s) reached.', vms)
            self.limits_reached_threads.add(threading.currentThread())

        for thread in threading.enumerate():
            if not thread.daemon or getattr(thread, 'type', None) == 'cron':
                # We apply the limits on cron threads and HTTP requests, longpolling requests excluded.
                if getattr(thread, 'start_time', None):
                    ...
                    if thread_execution_time > thread_limit_time_real:
                        _logger.info('Thread %s virtual real time limit (%ss) reached.', thread, thread_execution_time)
                        self.limits_reached_threads.add(thread)
        # Clean-up threads that are no longer alive
        # e.g. threads that exceeded their real time, but which finished before the server could restart.
    ...
    def stop(self):
        ...
        for thread in threading.enumerate():
            ...
            if (thread != me and not thread.isDaemon() and thread.ident != self.main_thread_id and
                ...
                while thread.isAlive() and (time.time() - stop_time) < 5:
        ...
        try:
            while self.quit_signals_received == 0:
                ...
                if self.limit_reached_time:
                    has_other_valid_requests = any(not t.daemon and t not in self.limits_reached_threads for t in threading.enumerate() if getattr(t, 'type', None) == 'http')
                    if not has_other_valid_requests or (time.time() - self.limit_reached_time) > SLEEP_INTERVAL:
                        # We wait there is no processing requests other than the ones exceeding the limits, up to 1 min,
                        # before asking for a reload.
                        self.reload()
                        # `reload` increments `self.quit_signals_received` and the loop will end after this iteration,
                        # therefore leading to the server stop.
                        # `reload` also sets the `phoenix` flag to tell the server to restart the server after shutting down.
                    else:
                        time.sleep(1)
                else:
