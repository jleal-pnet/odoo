...
class Partner(models.Model):
    ...
    def _execute_regular_channel_search_query(self, name, limit, excluded_partner_ids, channel):
        ...
        if has_restricted_group:
            query.append("AND GU.gid = " + str(channel.group_public_id.id))

        query.append("LIMIT %s")

        self.env.cr.execute(''.join(query), ("%s seconds" % DISCONNECTION_TIMER, "%s seconds" % AWAY_TIMER, name, tuple(excluded_partner_ids), limit))
