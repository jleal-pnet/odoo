...
class Partner(models.Model):
    ...
    def _notify_prepare_template_context(self, message, record, model_description=False, mail_auto_delete=True):
        ...
        company = record.company_id if record and hasattr(record, 'company_id') else user.company_id
        if company.website:
    ...
    def _notify(self, message, record, force_send=False, send_after_commit=True, model_description=False, mail_auto_delete=True):
        ...
        :param layout: Xml ID of a QWeb template used to encapsulate the message to send by
                       email;
        :param force_send: whether to send notification emails right now instead of using the
                           email queue;
        ...
        :param dict notif_values: values used to compute the notification process, containing
