...
class Message(models.Model):
    ...
    def _notify_compute_recipients(self, record, msg_vals):
        ...
        for pid, cid, pshare, ctype, notif, groups in res:
            if pid and pid == author_id:  # do not notify the author of its own messages
                continue
            if pid and notif == 'inbox':
                recipient_data['partners'].append((pid, pshare, notif, 'user', groups))
            elif pid:
                if not pshare and notif:  # has an user and is not shared, is therefore user
                    recipient_data['partners'].append((pid, pshare, 'email', 'user', groups))
                elif pshare and notif:  # has an user, is therefore portal
                    recipient_data['partners'].append((pid, pshare, 'email', 'portal', groups))
                else:  # has no user, is therefore customer
                    recipient_data['partners'].append((pid, pshare, 'email', 'customer', groups))
            elif cid:
                recipient_data['channels'].append((cid, ctype, notif, '', False))
    ...
    def _notify_recipients(self, rdata, record, msg_vals,
        ...
        :param rdata: dict containing recipients data: {
            'partners': list of tuples containing (),
            'channels': list of tuples containing (),
        }
        """
        self.ensure_one()
