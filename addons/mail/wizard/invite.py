    ...
    def add_followers(self):
        ...
        for wizard in self:
            ...
            if wizard.send_mail and wizard.message and not wizard.message == '<br>':  # when deleting the message, cleditor keeps a <br>
                ...
                self.env['res.partner'].with_context(auto_delete=True)._notify(message, [(pid, True, 'email', 'customer', []) for pid in new_partners.ids], document, force_send=True, send_after_commit=False)
                message.unlink()
