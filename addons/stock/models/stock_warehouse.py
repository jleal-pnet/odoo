...
class Warehouse(models.Model):
    ...
    mto_pull_id = fields.Many2one('stock.supply.rule', 'MTO rule')
    ...
    def create(self, vals):
        ...
        # create routes and push/Stock supply rule
        ...
        if vals.get('resupply_wh_ids') and not vals.get('resupply_route_ids'):
            for warehouse in warehouses:
                ...
                if to_remove:
                    ...
                    # TDE FIXME: shouldn't we remove Stock supply rule also ? because this could make them global (not sure)
    ...
    def _find_existing_rule_or_create(self, rules_list):
        ...
        for rule_vals in rules_list:
            existing_rule = self.env['stock.supply.rule'].search([
            ...
            if not existing_rule:
                self.env['stock.supply.rule'].create(rule_vals)
            else:
    ...
    def _create_or_update_reception_route(self, routes_data):
        ...
        for warehouse in self:
            ...
            # Stock supply rule for reception
    ...
    def _create_or_update_mto_pull(self, routes_data):
        ...
        routes_data = routes_data or self.get_routes_dict()
        Rule = self.env['stock.supply.rule']
        for warehouse in self:
            routings = routes_data[warehouse.id][warehouse.delivery_steps]
    ...
    def _check_delivery_resupply(self, new_location, change_to_multiple):
        ...
        if not change_to_multiple:
            ...
            # We need to delete all the MTO Stock supply rule, otherwise they risk to be used in the system
            Rule.search([
    ...
    def _check_reception_resupply(self, new_location):
        ...
        self.env['stock.supply.rule'].search([
    ...
    def _location_used(self, location):
        rules = self.env['stock.supply.rule'].search_count([
            '&',
...
class Orderpoint(models.Model):
    ...
    group_id = fields.Many2one(
        'stock.supply.group', 'Stock Supply Group', copy=False,
        help="Moves created through this orderpoint will be put in this stock supply group. If none is given, the moves generated by Stock supply rule will be grouped into one big picking.")
    company_id = fields.Many2one(
