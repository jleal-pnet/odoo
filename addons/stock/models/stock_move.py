        ...
        for move in self.filtered(lambda m: m.state in ['confirmed', 'waiting', 'partially_available']):
            ...
            if move.location_id.should_bypass_reservation()\
                ...
                if not move.move_orig_ids:
                    ...
                    available_quantity = self.env['stock.quant']._get_available_quantity(move.product_id, move.location_id, package_id=forced_package_id, owner_id=move.restrict_partner_id or None)
                    if available_quantity <= 0:
                        continue
                    taken_quantity = move._update_reserved_quantity(need, available_quantity, move.location_id, package_id=forced_package_id, owner_id=move.restrict_partner_id or None, strict=False)
