...
class StockSupplyGroup(models.Model):
    ...
    def run(self, product_id, product_qty, product_uom, location_id, name, origin, values):
        ...
        if not rule:
            raise UserError(_('No rule found. Please verify the configuration of your routes'))
    ...
    def _search_rule(self, route_ids, product_id, warehouse_id, domain):
        ...
        res = self.env['stock.supply.rule']
        if route_ids:
    ...
    def run_scheduler(self, use_new_cursor=False, company_id=False):
        """ Call the scheduler in order to check the running procurements (super method), to check the minimum Stock supply rule
