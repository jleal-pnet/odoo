...
class Picking(models.Model):
    ...
    move_ids_without_package = fields.One2many('stock.move', 'picking_id', string="Stock moves not in package", compute='_compute_move_without_package')
    ...
    def _compute_move_without_package(self):
        for picking in self:
            ...
            if picking.picking_type_entire_packs == False:
                ...
                for move in picking.move_lines:
                    if move.state != 'assigned' and not move.package_level_id:
                        move_ids_without_package |= move
                    elif move.state == 'assigned' and not move.package_level_id and any(not ml.package_level_id for ml in move.move_line_ids):
                        move_ids_without_package |= move
