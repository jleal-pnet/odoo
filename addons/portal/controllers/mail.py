...
class PortalChatter(http.Controller):
    ...
    def portal_message_fetch(self, res_model, res_id, domain=False, limit=10, offset=0, **kw):
        ...
        domain = expression.AND([domain, expression.normalize_domain(field_domain(request.env[res_model]) if callable(field_domain) else field_domain)])
        domain = expression.AND([domain, [('res_id', '=', res_id)]])
