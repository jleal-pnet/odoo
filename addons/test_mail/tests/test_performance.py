...
class TestAdvMailPerformance(TransactionCase):
    ...
    def test_adv_activity_full(self):
        ...
        with self.assertQueryCount(margin=1, admin=26, emp=39):  # test_mail only: 26 - 39
    ...
    def test_adv_activity_mixin(self):
        ...
        with self.assertQueryCount(margin=1, admin=28, emp=40):  # test_mail only: 28 - 40
    ...
    def test_message_assignation_email(self):
        ...
        with self.assertQueryCount(margin=1, admin=56, emp=71):  # test_mail only: 56 - 71
            record.write({
    ...
    def test_message_assignation_inbox(self):
        ...
        with self.assertQueryCount(margin=1, admin=37, emp=47):  # test_mail only: 37 - 47
            record.write({
                'user_id': self.user_test.id,
            })
    ...
    def test_message_post_one_email_notification(self):
        ...
        with self.assertQueryCount(margin=1, admin=46, emp=62):  # com runbot: 44 - 60 // test_mail only: 46 - 62
            record.message_post(
    ...
    def test_message_post_one_inbox_notification(self):
        ...
        with self.assertQueryCount(margin=1, admin=31, emp=41):  # com runbot 30 - 40 // test_mail only: 31 - 41
            record.message_post(
...
class TestHeavyMailPerformance(TransactionCase):
    ...
    def test_complex_message_post(self):
        ...
        with self.assertQueryCount(admin=79, emp=96):  # com runbot 77 - 94 // test_mail only: 79 - 96
            record.message_post(
    ...
    def test_complex_message_post_template(self):
        ...
        with self.assertQueryCount(admin=96, emp=115):  # com runbot 94 - 113 // test_mail only: 96 - 115
    ...
    def test_complex_tracking_assignation(self):
        ...
        with self.assertQueryCount(admin=59, emp=74):  # com runbot: 59 - 74 // test_mail only: 59 - 74
    ...
    def test_complex_tracking_subscription_create(self):
        ...
        with self.assertQueryCount(margin=1, admin=175, emp=202):  # test_mail only: 175 - 202
            rec = self.env['mail.test.full'].create({
                'name': 'Test',
    ...
    def test_complex_tracking_subscription_subtype(self):
        ...
        with self.assertQueryCount(margin=1, admin=113, emp=126):  # test_mail only: 113 - 126
            rec.write({
                'name': 'Test2',
                'umbrella_id': self.umbrella.id,
                })
    ...
    def test_complex_tracking_subscription_write(self):
        ...
        with self.assertQueryCount(margin=1, admin=119, emp=138):  # test_mail only: 119 - 136
            rec.write({
                'name': 'Test2',
                'umbrella_id': umbrella_id,
                'customer_id': customer_id,
    ...
    def test_complex_tracking_template(self):
        ...
        with self.assertQueryCount(admin=53, emp=68):  # test_mail only: 53 - 68
            rec.write({
                'name': 'Test2',
                'customer_id': customer_id,
                'user_id': self.env.uid,
