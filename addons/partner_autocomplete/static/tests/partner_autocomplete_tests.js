odoo.define('partner_autocomplete.tests', function (require) {
    ...
    QUnit.module('partner_autocomplete', {
        before: function () {
            var suggestions = [
                {name: "Odoo", website: "odoo.com", logo: "odoo.com/logo.png"}
            ];

            var suggestions_vat = [
                {name: "Odoo", vat: "BE0477472701"}
            ];
            ...
            testUtils.patch(AutocompleteCore, {
                ...
                getVatSuggestions: function (value) {
                    ...
                    var results = _.filter(suggestions_vat, function (suggestion) {
                        return (suggestion.vat.toLowerCase().indexOf(value.toLowerCase()) >= 0);
                    });
                    if (results && results.length) {
                        var vat_match = results[0];
                        this.getNameSuggestions(vat_match.name).then(function (suggestions) {
            ...
            this.data = {
                'res.partner': {
                    fields: {
                        ...
                        company_data_id: {string: "Company data ID", type: "integer", searchable: true},
                    },
                    records: [],
                    onchanges: {
    ...
    QUnit.test("Partner autocomplete : Company type = Company / Name search", function (assert) {
        ...
        assert.strictEqual($dropdown.length, 1, "There should a message when there is no suggestion");
    ...
    QUnit.test("Partner autocomplete : Company type = Company / VAT search", function (assert) {
        ...
        var $dropdown = form.$(".o_field_partner_autocomplete .dropdown-menu:visible");
        assert.strictEqual($dropdown.length, 1, "there should be an opened dropdown");
        assert.strictEqual($dropdown.children().length, 1, "there should be a message for no results with incomplete VAT number");
        ...
        $dropdown = form.$(".o_field_partner_autocomplete .dropdown-menu:visible");
        assert.strictEqual($dropdown.length, 1, "there should be an opened dropdown");
        assert.strictEqual($dropdown.children().length, 2, "there should be two propositions (1 for vat + 1 for name) for complete VAT number");
        ...
        $dropdown.find("a").eq(1).first().click();

        $input = form.$(".o_field_partner_autocomplete > input");
        assert.strictEqual($input.val(), "Odoo", "Input value should have been updated to \"Odoo\"");

        fields = this.data['res.partner'].fields;
        _compareResultFields(assert, form, fields, AutocompleteCore._createData);
