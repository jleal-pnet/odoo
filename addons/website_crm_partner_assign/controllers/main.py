...
class WebsiteAccount(CustomerPortal):
    ...
    def get_domain_my_lead(self):
        domain = request.env.ref('website_crm_partner_assign.action_lead_portal_domain').with_context(type='lead').run()
        return domain

    def get_domain_my_opp(self):
        domain = request.env.ref('website_crm_partner_assign.action_lead_portal_domain').with_context(type='opportunity').run()
        return domain

    def _prepare_portal_layout_values(self):
        values = super(WebsiteAccount, self)._prepare_portal_layout_values()
        lead_count = request.env['crm.lead'].search_count(self.get_domain_my_lead())
        opp_count = request.env['crm.lead'].search_count(self.get_domain_my_opp())
        values.update({
    ...
    def portal_my_leads(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        domain = self.get_domain_my_lead()
