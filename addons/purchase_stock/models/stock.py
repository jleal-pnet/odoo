...
class StockWarehouse(models.Model):
    ...
    buy_pull_id = fields.Many2one('stock.supply.rule', 'Buy rule')
    ...
    def create_routes(self):
        ...
        if self.buy_to_resupply:
            res['buy_pull_id'] = self._update_buy_rule()
        return res
            ...
            if vals.get("buy_to_resupply"):
                for warehouse in self:
                    if not warehouse.buy_pull_id:
                        vals['buy_pull_id'] = self._update_buy_rule()
            else:
                for warehouse in self:
    ...
    def _update_buy_rule(self):
        buy_pull_vals = self._get_buy_pull_rule()
        buy_pull = self.env['stock.supply.rule'].create(buy_pull_vals)
        return buy_pull.id
